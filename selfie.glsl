...
vec3 renderGirl( in vec2 p, in vec3 ro, in vec3 rd, in float tmax, in vec3 col, in float time )
{
    ...
    if( tm.y>0.0 )
    {
        ...
        {
            sss = 0.0;
            // iChannel0 is the 2D atlas of the 32^3 noise volume; this mirrors the
            // original sampler3D trilinear sample at raw normalized coords.
            col = vec3(0.81*noise1(iChannel0,uvw*192.0-0.5));
            ks *= 2.0;
